import java.util.InputMismatchException;			//Import der Input Mismatch Exception. Wichtig für das Menü
import java.util.Scanner; 							//Import eines Scanners. Wichtig für das Menü
import java.util.concurrent.ThreadLocalRandom;		// Import von ThreadLocalRandom. Wird genutzt um Zufallszaheln zu generieren


public class sudoku
{
   private static int count =0;						//Die Variable count zählt im Velauf des Programms die Anzahl möglicher Lösungen für ein Sudoku
   public static void main(String[] args)
   {
	
	Scanner in = new Scanner(System.in);
	int Number = 0;									//Diese Variable dient zur Auswahl, ob eine Sudoku generiert oder gelöst werden soll
	int[][] A = new int[9][9];						// Hier wird das Array A erstellt. Das Sudoku welches in dieses Array eingegeben wird kann mit dem Programm gelöst werden.						
	...
	System.out.println("                                                             ");													//Menü des Sudokus
	...
	while(true){
			try{
				...
				{ 
					System.out.println("Bitte gebe eine 1 oder eine 2 ein");
					String flush = in.next();   //Löscht Falsche-Eingabe, um Wiederholungen zu vermeiden.
				} 
	}
	System.out.println();
	System.out.println();
   ...
   public static void Generate(){																	//Funktion generiet das Sudoku
	   int[][] B = new int[9][9];
	   generate(0,0,B);																				//1. Ein vollständig ausgefülltes Sudokufeld wird in der generate-Funktion erstellt			
	   
	   while(true){
		   int[][] A=new int[9][9];																	//2.zufällige Zahlen werden aus dem Sudoku entfernt
		  ...
		  for(int i=0;i<A.length;i++){
			...
			for(int g = 0;g<9;g++){
	   		...
	   		count=0;
		   if(test(nextposy,nextposx,A)){															//3. testet wie viele Lösungen das SUdoku hat. Bricht ab wenn es nur noch eine gibz
			   break;
		   }
   	...
   	public static void Solve(int[][] A){															//Diese Funktion löst das Sudoku in Array A
   		int nextposx= 0;
   		int nextposy= 0; 																			//nächste freie Stelle wird gesucht			
   		nextpos:{
		for(int g = 0;g<9;g++){
			for(int h = 0;h<9;h++){
				if(A[g][h]== 0){
	...
	System.out.println("Ohne Lösung sieht es so aus:");												// ungelöstes Sudoku wird ausgegeben				
	Ausgabe(A);
	
	backtrack(nextposy,nextposx,A);																	// Sudoku wird gelöst
	if(count>0){
	System.out.println();
	System.out.println();
	System.out.println();
	System.out.println("Das Sudoku hat: "+ count + " Lösungen");									// Anzahl der möglichen Lösungen wird ausgegeben	
	}else{
	System.out.println();
	System.out.println();
	System.out.println();
	...
	public static boolean backtrack(int i, int j,int[][] A){
		...
		for(int x = 1;x<=9;x++){
			...
			nextpos:{																				// nächstes freies Feld wird gesucht				
				for(int g = i;g<9;g++){
					int b=0;
					if(g==i){
						b = j;
				...
				end = true;																			//Ende des Sudokus wurde erreicht, keine freine Felder mehr übrig
			}
	...
	public static boolean generate(int i, int j,int[][] A){											// wie Backtrack
		...
		shuffle(S);																					//Array wird gemischt um zufällige Sudokus zu generieren
	...
	public static boolean test(int i, int j,int[][] A){												// auch wie backtrack, bricht aber bei mehr als einer Lösung ab
		int nextposx= 0;
		int nextposy= 0;
		boolean end=false;
	...
	public static void Ausgabe(int[][] A){													//Diese Funktion gibt das Sudoku Array aus und fügt noch ein Gitternetz ein.
		for(int i = 0;i< 9;i++){
			if(i% 3== 0  && i != 0){
				System.out.println("------------");
				}
